...
export const WaitingRoom: React.FC<WaitingRoomProps> = ({
  ...
  if (loading) {
    return (
      <div className="ibot-waiting-room-bg flex h-screen flex-col items-center justify-center overflow-hidden p-6">
        <div className="relative">
          <div className="absolute -inset-5 rounded-full bg-emerald-200/50 blur-2xl animate-pulse" />
          <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl bg-white text-emerald-700 shadow-xl shadow-emerald-500/20">
  ...
  if (error || !details) {
    return (
      <div className="ibot-waiting-room-bg flex h-screen items-center justify-center overflow-hidden p-6">
  ...
  const completedChecks = [isOnline, micStatus === 'granted'].filter(Boolean).length;
  const setupPercent = Math.round((completedChecks / 2) * 100);
  const overviewItems = [
    'Keep this tab active during the interview.',
    'Use a quiet room and speak naturally into your microphone.',
      ...
      <header className="z-20 flex h-[76px] shrink-0 items-center justify-between border-b border-white/80 bg-white/80 px-4 shadow-sm shadow-slate-200/60 backdrop-blur-2xl sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-display text-base font-black text-slate-950">iBot</span>
            ...
            <p className="truncate text-xs font-semibold text-slate-500">Complete setup checks before joining.</p>
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          >
            ...
            {isReady ? 'Ready' : `${completedChecks}/2 checks`}
          </div>
        </div>
      </header>
      ...
      <main className="ibot-scrollbar min-h-0 flex-1 overflow-y-auto p-4 sm:p-5 lg:p-6">
        <div className="mx-auto grid min-h-full w-full max-w-7xl gap-5 lg:grid-cols-[minmax(0,1fr)_430px]">
          <section className="flex min-h-0 flex-col gap-5">
            <div className="ibot-hero-panel relative overflow-hidden rounded-2xl border border-white/80 p-5 shadow-xl shadow-slate-200/50 sm:p-6">
              <div className="relative z-10">
                ...
                <p className="mt-3 max-w-2xl text-sm font-medium leading-relaxed text-slate-600">
                  You are almost ready for the interview. Run the final checks below, then choose practice mode or start the live assessment.
                </p>
                ...
                <div className="mt-5 rounded-2xl border border-emerald-100 bg-white/90 p-4 shadow-sm">
                  ...
                  <p className="mt-1 text-lg font-black text-emerald-700">{details.assessment_title}</p>
                </div>
            ...
            <div className="ibot-panel p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs font-semibold text-slate-500">
                    Microphone and connection checks protect interview quality.
                  </p>
                </div>
                <button
                  onClick={() => setInstructionsOpen(true)}
                  className="inline-flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700 transition-all hover:bg-emerald-100 active:scale-[0.98]"
                >
                  Interview rules
              ...
              <div className="mt-5">
                <div className="mb-2 flex items-center justify-between text-xs font-black text-slate-500">
                  <span>{completedChecks} of 2 checks complete</span>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] font-semibold leading-relaxed text-amber-800">
                      Browser focus, audio input, and session continuity may be monitored for assessment integrity.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="ibot-start-panel flex flex-col gap-4 rounded-2xl border border-white/80 p-5 shadow-xl shadow-emerald-900/10 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/80 px-3 py-1 text-[10px] font-black uppercase text-emerald-700">
                  <Sparkles className="h-3 w-3" />
                  Final step
                </div>
                <p className="text-lg font-black text-slate-950">Ready to begin?</p>
                <p className={`mt-1 text-sm font-semibold ${isReady ? 'text-emerald-700' : 'text-red-600'}`}>
                  {isReady
                    ? 'Microphone and network checks are complete.'
                    : 'Enable microphone access and stay online to continue.'}
                </p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-2 rounded-xl border px-5 py-3 text-sm font-black transition-all active:scale-[0.98] ${
                    isReady
                      ? 'border-emerald-200 bg-white text-emerald-700 shadow-sm hover:-translate-y-0.5 hover:bg-emerald-50 hover:shadow-md'
                ...
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-sm font-black text-white transition-all active:scale-[0.98] ${
                    isReady
                      ? 'bg-slate-950 shadow-lg shadow-slate-900/20 hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-emerald-700/20'
          ...
          <aside className="flex min-h-0 flex-col gap-5">
            <div className="ibot-panel p-5">
              ...
              <button
                ...
                className="mt-4 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black text-slate-600 transition-all hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Retest Connection
              </button>
            </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  {micStatus !== 'granted' && (
                    <button
                      ...
                      className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white shadow-lg shadow-slate-900/10 transition-all hover:-translate-y-0.5 hover:bg-emerald-700 disabled:opacity-50"
                    >
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-2">
                        {!isRecording ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-200 bg-white px-3 py-2 text-xs font-black text-emerald-700 transition-all hover:bg-emerald-50 disabled:opacity-40"
                          >
      ...
      {instructionsOpen && (
        <ModalShell onClose={() => setInstructionsOpen(false)} maxWidth="max-w-xl">
          ...
          <div className="ibot-scrollbar max-h-[60vh] overflow-y-auto px-6 py-5 text-sm leading-relaxed text-slate-600">
            ...
            <div className="mt-5 flex gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-emerald-800">
          ...
          <div className="flex justify-end border-t border-slate-100 bg-slate-50/60 px-6 py-4">
            <button
              onClick={() => setInstructionsOpen(false)}
              className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white shadow-lg shadow-slate-900/10 transition-all hover:bg-emerald-700"
            >
              I Understand
            </button>
          </div>
        ...
        <ModalShell onClose={() => setConfirmStartOpen(false)} maxWidth="max-w-md">
          <div className="space-y-4 p-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm leading-relaxed text-slate-500">
                You will enter a proctored interview environment. Browser focus, audio input, and the assessment timeline may be monitored.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 rounded-xl bg-slate-950 px-4 py-3 text-xs font-black text-white shadow-lg shadow-slate-900/10 transition-all hover:bg-emerald-700"
              >
                Start Now
              </button>
            </div>
...
const InfoTile: React.FC<{ icon: React.ReactNode; label: string; value: string }> = ({
  ...
  <div className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm">
    <div className="mb-2 flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100">
      {icon}
    </div>
